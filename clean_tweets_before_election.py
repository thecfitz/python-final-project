...
filtered_tweets_before = []
tokenized_tweets_before = []

# read tweets from csv
with open('Trump_tweets_before_election2.csv', 'r', encoding="ISO-8859-1") as csv_file:
    tweet_reader = csv.reader(csv_file)

    # tokenize tweets and add to list
    for tweet in tweet_reader:
        for word in tweet:
            tokenized_tweets_before.append(word_tokenize(word))

    # clean unnecessary tweet elements
    for tweet in tokenized_tweets_before:
...
for row in range(len(tokenized_tweets_before)):
    tweet_lower = [tokenized_tweets_before[row][index].lower() for index in range(len(tokenized_tweets_before[row]))]
    tokenized_tweets_before[row] = tweet_lower
...
for tweet in tokenized_tweets_before:
    ...
    filtered_tweets_before.append(useful_tweets)

# define a function for counting days since announcing candidacy
def count_day_function():
    day_count = -1
    for i in range(len(filtered_tweets_before)):
        day = filtered_tweets_before[i][-2]
        if day != filtered_tweets_before[i-1][-2]:
            day_count += 1
        filtered_tweets_before[i].insert(-2,day_count)
    for tweet in filtered_tweets_before:
        tweet.pop(-2)
...
filtered_tweets_before = tweet_module.updated_tweet_list(filtered_tweets_before)

dict = tweet_module.common_keywords(filtered_tweets_before,dict)

primary_listb = tweet_module.word_frequency_list(dict)

pop_words = tweet_module.pop_words_list(policy_words, filtered_tweets_before)
